...
float zero_acc_x,zero_acc_z,zero_acc_y;
...
void MPU6050_setZero(){
	...
	for(i=0;i<250;i++){
		tmp_raw += MPU6050_getGyroX_raw();
	}
	tmp_raw /= 250;
	zero_gyro_roll = tmp_raw;
}

int16_t MPU6050_getGyroX_raw(){
...
float MPU6050_getGyroY_degree(){

	int16_t tmp;
	float degree;

	tmp = MPU6050_getGyroY_raw();
	degree = (tmp / 65.5 );
	return degree;
}
